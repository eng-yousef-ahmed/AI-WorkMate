...
export function evaluateAnalysisQuality(analysis: AnalysisDocument, transcript: TranscriptDocument): AnalysisQualityReport {
  ...
  const hallucinatedNames = uniqueStrings(
    [...analysis.decisions.flatMap((row) => [row.owner ?? ""]), ...analysis.tasks.map((row) => row.assignee ?? "")]
      .map((name) => name.trim())
      .filter((name) => name.length > 0 && !nameAppearsInTranscript(name, namedPeople, corpus)),
  );
  const matchedDecisions = ANALYSIS_QUALITY_MARKERS.decisions.filter((marker) =>
    analysis.decisions.some((row) => row.text.includes(marker)) && corpus.includes(marker),
  ).length;
  const matchedTasks = ANALYSIS_QUALITY_MARKERS.tasks.filter((marker) =>
    analysis.tasks.some((row) => row.text.toLowerCase().includes(marker.toLowerCase())) && corpus.toLowerCase().includes(marker.toLowerCase()),
  ).length;
  ...
  const summaryMentionsMeeting = /AI WorkMate|DATA_ROOT|llama\.cpp/i.test(analysis.summary);
...
function nameAppearsInTranscript(name: string, namedPeople: string[], corpus: string): boolean {
