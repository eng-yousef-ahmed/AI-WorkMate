...
- **WINDOWS-UNVERIFIED:** The full production path `WindowsNativeAudioProvider → NativeCaptureCoordinator → LocalRecordingCaptureEngine → LocalFirstStore → artifact journal → SQLite recording metadata → final media file` is exercised only by `npm run verify:windows-native-capture` on a real Windows host. This Linux host fail-closes. Do not label the path WINDOWS-VERIFIED until that command is run on Windows and its JSON `success`/`windowsVerified` fields are true.
