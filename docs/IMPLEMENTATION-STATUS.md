...
This checkout hardens the existing local-first storage foundation, adds the Phase 4 Microsoft 365 calendar discovery integration layer, implements the Phase 5 local recording/capture boundary, adds the Phase 6A native Windows capture source boundary, implements Phase 6B Windows native audio provider code with real Windows helper verification, integrates that path into `StorageRuntime` (Phase 6C), adds Windows DXGI screen and Windows Graphics Capture (WGC) window helper code with runtime verification commands, adds Phase 7A local transcription of committed AIWPCM recordings, Phase 7B production wiring for local whisper.cpp, and Phase 7C Windows runtime/model install plus verification support. It does not
implement real Teams/Zoom/Google Meet/browser automation or a Git-bundled Whisper binary/model, and it does not yet claim Windows-verified WGC window capture (the DXGI screen runtime verification was executed on a real Windows host; see the Windows native screen/window capture section). The persistent meeting store
...
- **REMAINING GAP:** Capture UI, mixed microphone+loopback sessions, Windows-verified WGC window helper execution, and Teams/Zoom automation.
...
- **IMPLEMENTED / TESTED / WINDOWS-VERIFIED (SCREEN runtime):** `npm run verify:windows-native-screen-capture` fail-closes off Windows (`windowsVerified: false`, isolated temp DATA_ROOT, no cloud). A real Windows host ran it with `--duration-ms=10000`: `success` true and `windowsVerified` true — real DXGI Desktop Duplication JPEG frames committed through the `StorageRuntime`/artifact journal pipeline with SQLite `COMMITTED`, meeting `PROCESSING`, and an `INCOMPLETE` abort journal with no recording row.
- **IMPLEMENTED / TESTED (WINDOW runtime, still WINDOWS-UNVERIFIED):** `npm run verify:windows-native-window-capture` exercises the real Windows Graphics Capture path against an actual enumerated HWND (never a mock/fake frame) for the requested `--duration-ms`, then commits the `aiwvid` JSONL JPEG artifact through the existing `StorageRuntime`/artifact journal pipeline and verifies: deterministic selection of a visible capturable window (helper enumeration excludes invisible/cloaked/tool/untitled windows; ordinary application windows are preferred over the shell desktop and candidates are ordered by numeric HWND), sequence ordering from 0, JPEG SOI framing, per-chunk and recording SHA-256, SQLite recording status `COMMITTED`, journal state `COMMITTED`, meeting status `PROCESSING`, final artifact existence with non-zero size, isolated workspace, untouched userData, and no cloud/network use. Abort verification leaves an `INCOMPLETE` journal with zero recording rows. If WGC delivers no frames for the selected window (minimized, protected/DRM, or otherwise ineligible), verification **fails clearly** with the window label/source in the failure message and never falls back to SCREEN capture. Linux fail-closes with `windowsVerified: false`. A first real Windows run failed at interop with `NATIVE_WINDOWS_API_INITIALIZATION_FAILED` / "Marshaling directives are invalid." (see the corrected-interop bullet below); the fix is committed and the real Windows execution remains to be re-run on the user's Windows host.
- **WINDOWS-VERIFIED: NO (WGC window runtime only)** — the sandbox cannot perform the real Windows GUI window capture. Limits: DXGI DDA needs an interactive desktop; WGC cannot capture protected content and may fail when minimized; JPEG ~5 fps max width 1280; not H.264; no mixed A/V.
- **IMPLEMENTED / CODE-VERIFIED (screen runtime WINDOWS-VERIFIED; WGC window runtime pending):** `native/windows-screen/Program.cs` was corrected against the exact package versions the helper pins (`Vortice.Direct3D11` 3.6.2, which pulls `SharpGen.Runtime.COM` 2.2.0-beta). Vortice 3.6.2 is the release that stopped mapping C++ `UINT` to C# `int`, so `IDXGIFactory1.EnumAdapters1`, `IDXGIAdapter1.EnumOutputs` and `IDXGIOutputDuplication.AcquireNextFrame` are now called with `uint`; the staging description uses the generated `Texture2DDescription.CPUAccessFlags` field plus `MipLevels`/`ArraySize`/`SampleDescription`; `MappedSubresource.RowPitch` is `uint` and is converted before the JPEG encoder; the desktop-resource block scopes the query-interfaced texture and the staging texture and releases the frame in a `finally`; display-loss detection uses the public `SharpGenException.ResultCode` compared with `Vortice.DXGI.ResultCode.AccessLost` because `SharpGenException.Descriptor` is private in SharpGen.Runtime 2.2.0-beta; the WGC `FrameArrived` handler parameter was renamed so the overflow-trim `TryDequeue(out _)` is a genuine discard (fixing CS1503) instead of binding the handler's object-typed `_` argument; and the WGC `IGraphicsCaptureItemInterop` ComImport methods no longer pass a projected WinRT type (`[MarshalAs(UnmanagedType.IInspectable)] out GraphicsCaptureItem`): the CLR cannot honor that directive against CsWinRT-projected types on .NET Core+, which raised "Marshaling directives are invalid." on the first real Windows WINDOW run. The interop interface now uses ABI-only signatures (`IntPtr CreateForWindow([In] IntPtr window, [In] ref Guid iid)` with the CLR mapping a failed HRESULT to `COMException`, translated to retryable `NATIVE_DEVICE_UNAVAILABLE` with the hex HRESULT), and the returned pointer is wrapped with the projected class's `GraphicsCaptureItem.FromAbi(...)` and released in a `finally` — the pattern CsWinRT's own generator and its documented COM interop guidance prescribe. Capture behaviour is unchanged: DXGI Desktop Duplication for displays, Windows Graphics Capture for HWNDs, `aiwvid` JSONL JPEG output, same error codes and exit codes. The helper was built and the SCREEN runtime verified on a real Windows host; `npm run build:native:win` + `npm run verify:windows-native-window-capture` still have to be re-run on a real Windows host before WGC window capture can be called WINDOWS-VERIFIED.
...
- **WINDOWS-VERIFIED / REMAINING GAP:** Phase 6B microphone and loopback helper capture was executed on a real Windows host. The DXGI screen helper runtime was executed on a real Windows host (`npm run verify:windows-native-screen-capture`); the WGC window helper runtime is implemented (`npm run verify:windows-native-window-capture`) but not yet Windows-executed.
...
The following commands completed successfully in the Linux sandbox after the Phase 7C and Windows screen/window verification work:

- `npm run lint` — **PASSED**, ESLint with zero warnings.
- `npm run typecheck` — **PASSED**.
- `npm test` — **PASSED: 190/190 tests**; its nested build also passed. The suite includes the fail-closed Linux checks for `npm run verify:windows-native-window-capture`, deterministic window-source selection tests, and a source-level guard that keeps the WGC `IGraphicsCaptureItemInterop` on ABI `IntPtr` signatures (no `[MarshalAs(UnmanagedType.IInspectable)]` projected-type marshaling, `GraphicsCaptureItem.FromAbi` + `COMException` handling). Whisper.cpp ggml models are validated with little-endian `GGML_FILE_MAGIC` (`0x67676d6c` on disk as `lmgg`), not ASCII `"ggml"`. Phase 7C Windows local transcription is **WINDOWS-VERIFIED** on a separate Windows host (see Phase 7C). Committed-transcript analysis is **TESTED** on Linux.
...
- No screen capture API was executed in this Linux sandbox. Real DXGI screen runtime verification was executed by the user on a separate Windows host and is recorded under "Windows native screen/window capture"; real Windows WGC window capture remains to be executed there. Linux native capture tests still use injected boundary/helper doubles. Real Windows microphone and WASAPI loopback helper capture was verified on a separate Windows host (Phase 6B) and is not re-executed in this Linux sandbox.
...
- Actual Windows `Program Files`/ACL behavior, junction/symlink semantics, Windows path parsing under the running Electron app, Windows-specific `statfs`/disk-full behavior, and real Windows WGC window capture require a Windows host. Microphone/loopback helper capture, Phase 6C StorageRuntime persistence, and the DXGI screen runtime verification are **WINDOWS-VERIFIED**.
...
- **No platform meeting recorder:** Microsoft calendar discovery, Teams identification, the local capture/storage boundary, native source abstraction/policy/coordinator, Windows native audio provider, DXGI screen/WGC window providers, and runtime integration are implemented. Real Windows microphone/loopback helper capture, Phase 6C StorageRuntime persistence, and the DXGI screen runtime verification are **WINDOWS-VERIFIED**; the WGC window runtime verification is pending a real Windows run of `npm run verify:windows-native-window-capture`. Teams/Zoom/Google Meet/browser automation remains out of scope.
