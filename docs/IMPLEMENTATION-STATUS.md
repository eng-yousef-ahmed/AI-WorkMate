...
- **IMPLEMENTED / CODE-VERIFIED (still WINDOWS-VERIFIED: NO):** `native/windows-screen/Program.cs` was corrected against the exact package versions the helper pins (`Vortice.Direct3D11` 3.6.2, which pulls `SharpGen.Runtime.COM` 2.2.0-beta). Vortice 3.6.2 is the release that stopped mapping C++ `UINT` to C# `int`, so `IDXGIFactory1.EnumAdapters1`, `IDXGIAdapter1.EnumOutputs` and `IDXGIOutputDuplication.AcquireNextFrame` are now called with `uint`; the staging description uses the generated `Texture2DDescription.CPUAccessFlags` field plus `MipLevels`/`ArraySize`/`SampleDescription`; `MappedSubresource.RowPitch` is `uint` and is converted before the JPEG encoder; the desktop-resource block scopes the query-interfaced texture and the staging texture and releases the frame in a `finally`; and display-loss detection uses the public `SharpGenException.ResultCode` compared with `Vortice.DXGI.ResultCode.AccessLost` because `SharpGenException.Descriptor` is private in SharpGen.Runtime 2.2.0-beta. Capture behaviour is unchanged: DXGI Desktop Duplication for displays, Windows Graphics Capture for HWNDs, `aiwvid` JSONL JPEG output, same error codes and exit codes. `npm run build:native:win` still has to be run on a real Windows host before any of this can be called WINDOWS-VERIFIED.
